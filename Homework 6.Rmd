...
```
Birthweight is hypothesized to be influenced by gestational agge, baby's length, mother's health, and socioeconomic factors. 
...
Compare the model to one using length at birth and gestational age as predictors.
```{r alt_model1, message = FALSE, warning = FALSE}
alt_model1 = lm(bwt ~ blength + gaweeks, data = birthweight_data)
```

Next, compare the model to one using head circumference, length, sex, and all interactions.
